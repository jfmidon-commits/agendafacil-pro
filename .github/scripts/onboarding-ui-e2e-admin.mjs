...
try {
  const create = await admin('/auth/v1/admin/users', {
    ...
    body: JSON.stringify({
      ...
      password,
      email_confirm: true,
      user_metadata: { name: 'Barbeiro UI E2E', phone: '51999990000' },
    }),
  });
  if (!create.ok) throw new Error(`Admin user creation failed: ${create.status} ${await create.text()}`);
  userId = (await create.json()).id;
  console.log('✓ Temporary Auth user created without email delivery');
  ...
  await page.locator('[name=email]').fill(`cliente-${id}@example.com`);
  ...
  const appointments = await admin(`/rest/v1/appointments?user_id=eq.${userId}&select=id,status,client_name,starts_at`).then(r => r.json());
  if (appointments.length !== 1 || appointments[0].status !== 'confirmed' || appointments[0].client_name !== 'Cliente UI E2E') {
    throw new Error(`Appointment persistence failed: ${JSON.stringify(appointments)}`);
  }
  console.log('✓ DB has exactly one confirmed appointment');

  await page.goto(`${APP}/dashboard`, { waitUntil: 'networkidle' });
  await page.locator('table').getByText('Cliente UI E2E', { exact: false }).waitFor({ timeout: 15000 });
  console.log('✓ Dashboard renders booked client');
} finally {
