...
export default async function PublicBookingPage({params}:{params:Promise<{slug:string}>}){
  ...
  return <main><div className="card" style={{maxWidth:760,margin:"20px auto"}}><span className="badge">Agendamento online</span><h1>{profile.business_name}</h1>{profile.description&&<p className="muted">{profile.description}</p>}<BookingForm slug={profile.slug} services={services||[]}/></div></main>;
}
